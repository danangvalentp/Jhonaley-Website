...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $buyerWhatsapp = $_POST['buyerWhatsapp'] ?? '';
    $usernamePanel = $_POST['usernamePanel'] ?? 'Tidak Disebutkan'; // NEW: Get usernamePanel
    $ram = $_POST['ram'] ?? 'Tidak Disebutkan';
    ...
    // Tambahkan usernamePanel ke pesan invoice jika diperlukan
    $invoiceMessageForBuyer = "Halo! Terima kasih atas pembelian Panel Pterodactyl Anda.\n\n" .
                              "Berikut detail pesanan Anda:\n" .
                              "👤 Username Panel: {$usernamePanel}\n" . // NEW: Add usernamePanel to invoice
                              "💾 RAM: {$ram}\n" .
                              "🔢 Jumlah Panel: {$quantity}\n" .
    ...
    $telegramMessageText = "✨ *Pemesanan Panel Baru!* ✨\n\n" .
                           "📞 *WA Pembeli:* `{$buyerWhatsapp}`\n" . // Nomor asli dari input
                           "👤 *Username Panel:* `{$usernamePanel}`\n" . // NEW: Add usernamePanel to Telegram message
                           ...
                           "Harap segera proses pesanan panel ini.";
        ...
        if (strpos($photoMimeType, 'image/') === 0) {
            ...
            if ($telegramResponse === false) {
                ...
                if ($decodedTelegramResponse['ok']) {
                    $response = ['status' => 'success', 'message' => 'Bukti pembelian panel berhasil dikirim. Notifikasi admin dan invoice pembeli telah dikirim.'];
                } else {
